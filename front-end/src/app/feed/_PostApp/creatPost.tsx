...
const CreatePost = () => {
  ...
  const fileInputRefs = useRef<{
    photos: HTMLInputElement | null;
    videos: HTMLInputElement | null;
    documents: HTMLInputElement | null;
  }>({
    photos: null,
    videos: null,
    documents: null
        ...
        <div className="flex mt-4 justify-between">
          <div className="flex space-x-2">
            <PostButton
              ...
              label="Photo"
              color="text-sky-500"
              onClick={() => fileInputRefs.current.photos && fileInputRefs.current.photos.click()}
            />
            <PostButton
              icon={<Video className="h-4 w-4" />}
              label="Video"
              color="text-green-500"
              onClick={() => fileInputRefs.current.videos && fileInputRefs.current.videos.click()}
              />
            <PostButton
              icon={<FileText className="h-4 w-4" />}
              label="Document"
              color="text-amber-500"
              onClick={() => fileInputRefs.current.documents && fileInputRefs.current.documents.click()}
              />
...
const PostButton: React.FC<PostButtonProps> = ({ icon, label, color, onClick }) => (
  <button
    type="button"
