...
export const ProfileCard = ({ className = "", setProfileOpen }: { className?: string; setProfileOpen: () => void }) => {
        ...
        <div className="mt-4">
          <div className="flex items-center justify-between mb-1">
            ...
            <span className="text-xs font-medium text-white">
              ...
              if (user.profileImageURL) completedFields++;
              if (user.name) completedFields++;
              if (user.title) completedFields++;
              if (user.posts?.length?? 0 > 0) completedFields++;
              if ((user.followers?.length ?? 0) > 0 || (user.following?.length ?? 0) > 0) completedFields++;
