...
export const ChatProvider = ({ children }: { children: React.ReactNode }) => {
 ...
 const { websocket = null, loading4 } = useSocket(activeChat?.id as string);
 ...
 useEffect(()=>{
  if(!isLoading && rooms && !isLoading4&& currentUser){
   const userChats: ChatItem[] = rooms
    ...
    return {
     ...
     name:
      room?.users
        ?.filter((user) => user && user.name !== currentUser.name)[0]
        ?.name ?? "",
     avatar: room.users?.filter((user) => user && user.name !== currentUser.name)?.[0]?.profileImageURL ?? "",
      ...
      .map((message) => ({
        ...
        reactions: message.reactions
          ...
          .map((reaction) => ({
            ...
            count: message.reactions?.filter((r) => r?.type === (reaction?.type ?? "")).length ?? 0,
            reacted: message.reactions?.some((r) => (r?.type ?? "") === (reaction?.type ?? "") && r?.author?.name === currentUser.name) ?? false,
          })) ?? [],
 ...
 const sendSignalingData = (
  ...
  if (websocket && websocket.readyState === WebSocket.OPEN && activeChat) {
    ...
    console.error('WebSocket not connected or activeChat is null, cannot send signaling data');
  ...
  if (websocket) {
    websocket.onmessage = (event: MessageEvent) => {
      ...
      if(msg.type==="message_in_room"){
       ...
       const room = chats.find((chat) => chat.id === msg.roomId);
       if (!room) return;
       const message = (room.messages ?? []).find((message) => message && message.id === msg.messageId);
       if (!message) return;
       const reactions = message.reactions || [];
       ...
       setActiveChat((prevChat) => {
        if (!prevChat || !message) return prevChat;
        const updatedChat = {
         ...prevChat,
